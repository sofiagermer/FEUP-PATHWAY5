...
%@  @strong{Please note:} @code{library(bdb)} has been deprecated. See
%@  @code{library(lmdb)} for a replacement with similar feature set.
%@  
...
%@  Otherwise, we choose the spec with the most @code{+} positions in
