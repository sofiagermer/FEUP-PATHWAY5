...
automaton(CtrArgs, CtrTuple, Signature, SourcesSinks, Arcs0, Counters1, Initial1, Final1, Options) :-
	...
	automaton_common(CtrArgs, CtrTuple, Signature, SourcesSinks, Arcs0, Counters1, Initial1, Final1, Options, Goal).

% entrypoint from regular/2
automaton_common(CtrArgs, CtrTuple, Signature, SourcesSinks, Arcs0, Counters1, Initial1, Final1, Options, Goal) :-
	(   foreach(Opt,Options),
	    fromto(f(_,_,_,[]),Opt1,Opt2,Opt3),
	    param(Goal)
	do  automaton_option(Opt, Opt1, Opt2, Goal, 9)
	),
	Opt3 = f(Nodes3,[S1|StateVars],[Initial1|CtrSeq],Ann),
	(   foreach(Arc0,Arcs0),
	    foreach(Arc,Arcs1),
	    param(Counters1)
	...
	append([Nodes1a,Nodes1b,Nodes1c], Nodes1),
	sort(Nodes1, Nodes2),
	(   foreach(N,Nodes2),
	    foreach(N-J,Nodes3),
	    count(J,1,_)
	...
	sort(Arcs2, Arcs3),
	automaton_map_arcs_case(CtrTuple, Counters1, Counters1a, Arcs3, Extension3, Extension4, Clause),
	...
	table_checked(4, ArgsLs, Extension, leftmost, default, Goal).
...
automaton_map_arcs_case(CtrTuple, Counters0, Counters1, Arcs, Extension3, Extension4, Clause) :-
	...
	(   foreach(arc(Bf,Bv,Bt,Be),Arcs),
	    foreach([Bf,Bv,Bt],Extension3),
	    foreach([Bf,Bv,Bt,ID],Extension4),
	    param(Leaf2ID)
