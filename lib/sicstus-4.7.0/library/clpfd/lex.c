	       ...
	       Dvar var)
{
  ...
  for (j=nvars-1; j>=0; j--) {
    ...
    int dom_intersects =
      (dvar_intersect_set(var+j,valueset));
	...
	if (fd_intersect((ubp ? fd_intersection_interval(wam, dvar_set(var+j),Inf,FDdecr(elt))
  ...
  for (j=alpha; j<nvars; j++) {
    TAGGED elt = MakeSmall(vec[j]);
    int dom_intersects = dvar_intersect_set(var+j,valueset);
	    ...
	    TAGGED *global)
{
  ...
  if (pdata->stamp==state_stamp) { /* incremental */
    ...
    DerefArg(pdata->among_spec, tstate, 5);
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    pdata = Palloc(struct lexc_data, total_size, handle); /* GC */
  ...
  pdata->state[i] = 1; 
  DerefArg(pdata->among_spec, X(0), 5);
  ...
  /* entailment of among conditions -- must check before dvar_pruning_done() */
