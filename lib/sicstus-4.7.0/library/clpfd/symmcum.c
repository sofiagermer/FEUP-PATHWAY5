...
#define SIZEMACH(T)  dvar_value_count(wam,DVMACH(T))
...
ti_max_length_intersection(Wam wam, SP_integer low,SP_integer up,TASK t)
{
  ...
  if (min<=max &&
      dvar_intersect_interval_l(DVORG(t), min, max))
    return max_inter;
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    FdMemRequireHeap1(ntasks*5*INT_ATTRIBUTE_SIZE,handle,3);
    ...
    pdata = Palloc(struct symmcum_data, total_size, handle); /* GC */
    fd.gdata = pdata;
    ptr = (char *)(pdata+1);
    ...
    for (i=0; i<ntasks; i++) {
      ...
      DomFromAttr(tmp1,tmp1); /* dom/4 term */
