...
bc_alldiff_alloc(Wam wam, int nvars, int nrows, int total_rows, TAGGED handle)
{
  ...
  pdata = Palloc(struct alldiff_data, total_size, handle); /* GC */
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    pdata = bc_alldiff_alloc(wam, nvars, 0, 0, handle); /* GC, clobbers tvec */
    DerefArg(tvec,X(0),1);		/* get Vec */
    for (i=0; i<nvars; i++) {
      DerefCar(telt,tvec);
      DerefCdr(tvec,tvec);
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    while (TagIsLST(tvec)) {	/* count terms */
      ...
      DomFromAttr(telt,telt); /* dom/4 term */
    ...
    pdata = bc_alldiff_alloc(wam, nvars, nrows, total_rows, handle); /* GC, clobbers tvec */
    DerefArg(tvec,X(0),1);		/* get Vec */
    for (i=0; i<nvars; i++) {
      TAGGED telt;
      DerefCar(telt,tvec);
    ...
    pdata->lia.lhs[j++] = i;
    FdMemRequireHeap(nrows,EVAL_ARITY); /* GC */
    h = w->global_top;
    for (i=nrows-1; i>=0; i--)
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    total_size =
      ...
      2*nvars*sizeof(int) +
      nvars;
    pdata = Palloc(struct atleast_le_data, total_size, handle); /* GC, clobbers tvec */
