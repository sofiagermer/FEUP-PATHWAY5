...
static void print_fdset(Wam wam, TAGGED set)
{
  fprintf(stderr, "[");
  while (TagIsLST(set)) {
    TAGGED interval = CTagToCar(set);
    TAGGED head = CTagToCar(interval);
    TAGGED tail = CTagToCdr(interval);
    set = CTagToCdr(set);
    if (TagIsSmall(head) && TagIsSmall(tail)) {
      fprintf(stderr, "[%ld|%ld],", GetSmall0(head), GetSmall0(tail));
      SP_ASSERT(Tle(head,tail));
    }
    else if (TagIsSmall(head))
      fprintf(stderr, "[%ld|sup],", GetSmall0(head));
    else if (TagIsSmall(tail))
      fprintf(stderr, "[inf|%ld],", GetSmall0(tail));
    else
      fprintf(stderr, "[inf|sup],");
  }
  fprintf(stderr, "]\n");
}
#endif
