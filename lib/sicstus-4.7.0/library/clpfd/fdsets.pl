...
empty_interval(Min, Max) :-
	\+fdset_interval(_, Min, Max).


%   fdset_interval(?Set, ?Min, ?Max)
%   is true when Set is an fdset which is the nonempty interval Min..Max.
...
fdset_member(Elt, Set) :-
	...
	Set = [[A|B]|R],
	indomain_fast(R, A, B, Elt).

:- indomain_fast/4 is nondet.
indomain_fast([], A, A, V) :- !,
	V = A.
