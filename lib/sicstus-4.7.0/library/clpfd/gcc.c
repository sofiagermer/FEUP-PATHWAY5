	  ...
	  TAGGED handle)
{
  ...
  struct gcc_graph *pdata =
    Palloc(struct gcc_graph, total_size, handle); /* GC */
		   ...
		   SP_BOOL incremental)
{
  ...
  for (i=pdata->nvaltargets+1; i<pdata->nactive-1; i++) {
    v = ACTIVE(i);
    if (!incremental || dvar_value_count(wam,DVAR(v)) < IN_DEGREE(v)) {
	...
	for (j=0; j<degree; j++) {
	  ...
	  if (regret[0] > threshold[0] ||
	      ...
	      (cost_has_hole && regret[0]+regret[1]>0 &&
	       !dvar_intersect_interval_l(DVAR(0),
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    FdMemRequireHeap1((novars+novals)*INT_ATTRIBUTE_SIZE,handle,EVAL_ARITY);    
    fd.gdata = pdata = gcc_alloc(wam, novars, novals, flags, handle); /* GC, clobbers tlvec */
  ...
  for (i = 0; i < pdata->nvartargets; ++i) {
    ...
    if (live > 1)
      ent = 0;
    if (live != dvar_value_count(wam,dv))
