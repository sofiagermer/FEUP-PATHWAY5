...
/*
  '$fd_linear_reif'(+State0, -State, -Actions).
  State = state(CX,Op,RHS,Reif,Nground,TrailTop,Handle,Stamp) where CX are all non-ground
  Op in [1,2,3,4,5,6] means [#<,#=<,#=,#\=,#>,#>=].
  See enum rel.
*/
void SPCDECL
prolog_fd_linear_reif(Wam wam,
		      SP_term_ref State0,
		      SP_term_ref State,
		      SP_term_ref Actions)
{
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    pdata = Palloc(struct arith_data, 2*sizeof(struct dvar), handle); /* GC */
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    pdata = Palloc(struct arith_data, 3*sizeof(struct dvar), handle); /* GC */
    pdata->destructor = arith_destructor;
    pdata->nrefs = 6;
    FD_STORE_SPENV(pdata->spenv);
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    pdata = Palloc(struct arith_data, 4*sizeof(struct dvar), handle); /* GC */
    pdata->destructor = arith_destructor;
    pdata->nrefs = 6;
    FD_STORE_SPENV(pdata->spenv);
    pdata->refbase = SP_alloc_globrefs(6);
...
#include "quo-iteration.ic"
  /* extra reasoning for fixed X */
  if (dvar_is_integer(dvx) && (dvar_min_l(dvx) < -2 || dvar_min_l(dvx) > 2)) {
    SP_integer x = dvar_min_l(dvx);
    SP_integer absx = x<0 ? -x : x;
    SP_integer a = absx/2 + 1;
    SP_integer b = absx - 1;
    if (dvar_prune_interval_l(dvz,a,b)<0 || dvar_prune_interval_l(dvz,-b,-a)<0)
      goto ret;
  }
...
#include "div-iteration.ic"
  ...
  if (dvar_is_integer(dvx) && (dvar_min_l(dvx) < -2 || dvar_min_l(dvx) > 2)) {
    ...
    SP_integer a = absx/2 + 1;
    SP_integer b = absx - 1;
    SP_integer decr = absx % 2;
    if (dvar_prune_interval_l(dvz,a,b)<0 || dvar_prune_interval_l(dvz,-b,-a-decr)<0)
      goto ret;
  }
  if (dvar_is_integer(dvx) && dvar_is_integer(dvy) && dvar_is_integer(dvz)) {
...
#include "mod-iteration.ic"
  ...
  /* extra reasoning for fixed X */
  if (dvar_is_integer(dvx)) {
    SP_integer x = dvar_min_l(dvx);
    if (x > 1) {
      SP_integer a = (x+1)/2;
      SP_integer b = x - 1;
      if (dvar_prune_interval_l(dvz,a,b)<0)
	goto ret;
    } else if (x < -1) {
      SP_integer a = (x-1)/2;
      SP_integer b = x + 1;
      if (dvar_prune_interval_l(dvz,b,a)<0)
	goto ret;
    }
  }
  if (dvar_is_integer(dvx) && dvar_is_integer(dvy) && dvar_is_integer(dvz)) {
