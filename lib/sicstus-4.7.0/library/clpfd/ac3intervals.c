			      ...
			      SP_term_ref state_ref)
{
    ...
    
  {
    ...
    h[2] = TaggedZero;
    w->global_top = h+3;
    pdata = Palloc(struct ac3intervals_data, 0, h[1]); /* GC */
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    pdata = Palloc(struct ac3intervals_data, extra_size, handle); /* GC */
    ptr = (char *)(pdata+1);
    pdata->dvar = (Dvar)ptr;
			    ...
			    struct ac3element_data *pdata)
{
  ...
  while (tmin==Sup && l<pset->nbyvals) {
    if (dvar_intersect_interval_t(YVAR, pset->lit.a[l], pset->lit.b[l]) &&
			    ...
			    SP_term_ref state_ref)
{
  ...
  struct ac3element_data *pdata;
  {
    ...
    h[2] = TaggedZero;
    w->global_top = h+3;
    pdata = Palloc(struct ac3element_data, 0, h[1]); /* GC */
    pdata->destructor = ac3intervals_destructor;
    FD_STORE_SPENV(pdata->spenv);
    pdata->refbase = NULL;
    pdata->pset = pset;
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    pdata = Palloc(struct ac3element_data, extra_size, handle); /* GC */
    ptr = (char *)(pdata+1);
    pdata->dvar = (Dvar)ptr;
    ptr += 2*sizeof(struct dvar);
    SP_ASSERT(ptr==(char *)(pdata+1)+extra_size);
