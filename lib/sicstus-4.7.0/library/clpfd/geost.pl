...
geost(Objects1, Sboxes1, Options, Rules) :-
	...
	geost_lex_chains(Objects1r, Chains),
...
geost_lex_chains(Objects, Chains) :-
	(   foreach(Chain,Chains),
	    param(Objects)
	do  (   foreach(Oid,Chain),
		foreach(Orig,Origs),
		fromto(Objects,Objects1,Objects2,_)
	    do  selectchk(object(Oid,_,Orig,_), Objects1, Objects2)
	    ),
	    lex_chain(Origs, [hidden(true)])
	).
...
geost_objects([],            [],              [], _, _, _) --> [].
geost_objects([Object1|Os1], [Object1r|Os1r], [object(Oid,Sid1-Sid2,Origin2)|Os2], SidSet, Goal, K) -->
