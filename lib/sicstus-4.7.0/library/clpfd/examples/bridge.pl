...
/* dichotomic labeling of start times until each task has a compulsory part */
...
make_disj([[_H,R]|T], K, DType) -->
	{el_list(R, K, R1)},
	(   {DType==ti}
	->  (   foreach(task(Orig,Dur,_),R1),
		foreach(task(Orig,Dur,_,1,0),R2)
	    do  []
	    ),
	    {cumulative(R2, [/*global(true)*/])},
	    [R1]
	;   (   fromto(R1,[T1|R2],R2,[])
	    do  (   foreach(T2,R2),
		    param(T1)
