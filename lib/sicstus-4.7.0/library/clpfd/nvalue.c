	     ...
	     TAGGED handle)
{
  ...
  struct nvalue_data *pdata =
    Palloc(struct nvalue_data, total_size, handle); /* GC */
		      ...
		      SP_BOOL incremental)
{
  ...
  for (i=0; i<pdata->nvartargets; i++) {
    ...
    if (!incremental || dvar_value_count(wam,DVAR(var-nvals)) < IN_DEGREE(var)) {
...
/*    X(0) = RefTerm(State0); */
  ...
  if (!IsVar(handle)) {		/* got [Flag | '$free'(Ptr)] */
    ...
    pdata = nvalue_alloc(wam, nvars, nvals, handle); /* GC, clobbers tlvec */
