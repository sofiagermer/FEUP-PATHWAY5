...
:- vertices_edges_to_ugraph/3 is det.
...
:- top_sort/2 is semidet.       % failure (for cyclic graphs) may be an intentional case.
...
:- max_path/5 is semidet.       % failure (no path from V1 to V2) may be expected.
...
:- min_path/5 is semidet.       % failure (no path from V1 to V2) may be expected.
...
:- min_tree/3 is semidet.       % "if it exists"
