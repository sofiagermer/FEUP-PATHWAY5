...
mzn2fzn(MznFile0, Options, GoalArg, FznOptions, CleanUp0) :-
	...
	append(['--compile','--solver','org.minizinc.mzn-fzn', % These args make it emulate mzn2fzn
		'--output-fzn-to-file',file(FznFile),
		'--output-ozn-to-file',file(OznFile),
		'-I',file(LibDir),
		'-I',file(MznFileDir)
	       |Parameters],
	       [file(MznFile)], Args),
	process_create(MiniZinc, Args, [stdin(null),
					stderr(pipe(StdErr)),
					wait(ExitCode)]),
...
mzn_parameters([optimise(false)|Os], ['--no-optimize'|Ds], Cs) :- !,
	mzn_parameters(Os, Ds, Cs).
mzn_parameters([optimize(false)|Os], ['--no-optimize'|Ds], Cs) :- !,
	mzn_parameters(Os, Ds, Cs).
...
mzn_parameters([data_file(File0)|Os], ['--data', File|Ds], Cs) :- !,
	...
	mzn_parameters(Os, Ds, Cs).
mzn_parameters([option(O)|Os], [O|Ds], Cs) :- !,
	mzn_parameters(Os, Ds, Cs).
