...
%  Type is an atomic type.  See the discussion above for the definition of
...
:- atomic_type/1 is det.
atomic_type(Type) :-
	atomic_type(Type, _, _, _).

:- atomic_type/2 is det.
:- atomic_type/2 is documented_as(atomic_type/1).
atomic_type(Type, Primitive_type) :-
	atomic_type(Type, Primitive_type, _, _).

:- atomic_type/3 is det.
:- atomic_type/3 is documented_as(atomic_type/1).
atomic_type(Type, Primitive_type, Size) :-
	atomic_type(Type, Primitive_type, Size, _).

:- atomic_type/4 is det.
:- atomic_type/4 is documented_as(atomic_type/1).
atomic_type(Type, Primitive_type, Size, Alignment) :-
	atom(Type),
