...
other_msg(blame_on(Ancestor)) -->
	goal(Ancestor).
%% ^C handling
other_msg(interrupt_help) -->
	['Please enter a valid interrupt option (''h'' for help).'-[],nl].
...
/*
   ...
   This code duplication is implemented by extracting the lines ranges
   delimited by BEGIN...END.
*/

:- multifile query_class/5.
% BEGIN su_msgs.pl (See Bips/Makefile)
...
% END su_msgs.pl (See Bips/Makefile)
...
query_abbreviation(interrupt, '',
		   [abort-"a",
		    ...
		    native_ancestors(-1)-"G", % undocumented
		    empty-[0'\n],
		    help-"h?"
		    ]) :- !.
% END su_msgs.pl (See Bips/Makefile)
...
% BEGIN su_msgs.pl (See Bips/Makefile)
% This is the only clause of query_map_next_solution/4 that is copied into msgs.pl
query_map_next_solution(Char, _Chars, success, Answer) :-
	member(Answer-Abbrev, [yes-";n", no-[-1,0'\n,0'y], break-"b", '$backtrace'-"T"]),
	member(Char, Abbrev), !.
% END su_msgs.pl (See Bips/Makefile)
% The following clauses of query_map_next_solution/4 are not copied into msgs.pl
...
% BEGIN su_msgs.pl (See Bips/Makefile)
% A simplified command handling in msgs.pl uses a subset of these clauses.
...
standard_dbg_command(0'G,  A, native_ancestors(A),   pos_arg(-1)). % undocumented
