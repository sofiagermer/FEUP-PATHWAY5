 ...
 * multiple solutions to a query. This class is never instantiated explicitly by
 * the user.
 * 
 * <p>
 * You should not inherit from <code>SPQuery</code>.
 *
...
public class SPQuery implements Query {
    ...
    // cut/close/next and when returning to Prolog from Java (see
    // SICStus.(Meta)Glue_Push/PopContext)
    SPTerm term_stack_mark;

    long term_stack_index_mark = 0; // [PM] 3.8.7 SPRM 2495 SP_reset_term_refs(mark) when the query is closed/cut by
                                    // prolog runtime
    ...
    void dumpQueryStack() {
        ...
        for (x = this; x != null; x = x.next) {
            SICStus.dbgPrintln("Query " + x.superToString() + " term_stack_mark="
                    + ((x.term_stack_mark == null) ? "NULL" : x.term_stack_mark.superToString()) + " freeList="
                    + x.freeList);
        }
    }
    ...
    void setContext(SPQuery c) throws SPException {
        synchronized (sp) {
            if (c == this.next) {
                context = c;
            } else {
    ...
    void popToTermStackMark() {
        ...
        /*
         * { synchronized (sp) { // Do not attempt SPTerm toString here (e.g., for
         * debug) // System.out.println("term_stack_mark==" +
         * term_stack_mark.nativeTermRef); while (sp.term_stack != term_stack_mark) { //
         ...
         * SPTerm tmp = sp.term_stack; sp.term_stack = tmp.next; tmp.invalidate(); } } }
         */
    }
    ...
    void reuseTermRef(long termRef) throws SPException {
        if (sp.debugging()) {
            SICStus.dbgPrintln("reuseTermRef: termRef=" + termRef + ", freeList = " + freeList);
            if (sp.debugging(2))
                SICStus.dbgPrintln("reuseTermRef: this=" + this.toString());
            if (sp.debugging(2))
                SICStus.dbgPrintln("reuseTermRef: term_stack_mark=" + term_stack_mark);
        }

        if (term_stack_mark != null) {
            // if (sp.debugging(2)) SICStus.dbgPrintln("reuseTermRef: (term_stack_mark !=
            // null)");
            ...
            // if (sp.debugging(2)) SICStus.dbgPrintln("reuseTermRef: tmp=" + tmp + ",
            // termRef=" + termRef);

            // extra error check.
            if (!(termRef > tmp)) { // tmp belongs to the next choice point so termRef should be strictly newer
                String msg = "** ERROR: Internal Jasper Error: Trying to reuse a too old SP_term_ref " + "(reuse "
                        + termRef + ", " + "mark=" + tmp + ")";
                if (sp.debugging()) {
                    SICStus.dbgPrintln(msg);
                    sp.term_stack.dumpTermStack();
            ...
            if (freeList == null) {
                ...
                // if (sp.debugging(2)) SICStus.dbgPrintln("reuseTermRef: created freeList=" +
                // freeList);
            }
            if (!freeList.add(Long.valueOf(termRef))) {
                // invariant violation, was already present!
                String msg = "** ERROR: Internal Jasper Error: termRef(" + termRef + ") already in freeList = "
                        + freeList;
                if (sp.debugging())
                    SICStus.dbgPrintln(msg);
                throw new SPException(sp, 0, msg);
    ...
    long getTermRef() {
        if (sp.debugging())
            SICStus.dbgPrintln("getTermRef: freeList = " + freeList);
        if (freeList == null || freeList.isEmpty()) {
            return SPTerm.SP_ILLEGAL_TERM_REF;
        }
        Long x = freeList.pollFirst();
        return x.longValue();
    }
     ...
     * <code>SPTerm</code> objects created since this query was created
     * <p>
     ...
     * to this query (i.e. data referred to by {@link se.sics.jasper.SPTerm SPTerm}
     ...
     * 
     * @throws SPException
     *             If a Prolog exception was thrown during the query.
     * @return False if there are no more solutions, True otherwise.
     */
    @Override
    public synchronized boolean nextSolution() throws SPException {
     ...
     * {@link se.sics.jasper.SICStus#openQuery openQuery} was called. If the query
     * is not the most recently opened, all still open queries opened after this
     ...
     * operations on it will throw an exception. In particular you should not call
     * <code>close</code>, <code>cut</code> or <code>nextSolution</code> on an
     * invalidated query.
     * <p>
     * Invalidates all SPTerm objects created since this query was created If you
     * need to keep data created by the query (i.e. data referred to by
     * {@link se.sics.jasper.SPTerm SPTerm} objects), you need to convert it to Java
     ...
     **/
    @Override
    public synchronized void cut() throws SPException {
