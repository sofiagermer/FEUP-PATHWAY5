...
/**
 * {@code PrologSession} handles the connection with the Prolog Server.
...
public class PrologSession {
    ...
    /**
     * The {@code HttpSession} session attribute name.
     */
    ...
    /**
     * Creates a new {@code PrologSession} instance with default Prolog server
     * settings.
     */
    ...
    /**
     * Sets the timeout in milliseconds before the connection to the Prolog server
     * is reset (when a query is not answered). Setting the timeout to {@code 0}
     * will disable timeouts for this prolog session. Default is 2000 milliseconds.
     *
     * When a connection times out it will be closed silently.
     *
     * @param timeout
     *            {@code int} timeout in milliseconds or {@code 0} to disable
     *            timeouts
     */
    ...
    /**
     * Returns the {@code PrologSession} registered in JNDI with the given name. Use
     * this method in application servers where services are registered using JNDI.
     * Please note: the application server must be configured to register the
     * {@code PrologSession} with the given name for this method to work.
     *
     * @param name
     *            the name of the prolog session
     * @return the named prolog session or {@code null} if no such session could be
     *         found
     */
    ...
    /**
     * Returns the {@code PrologSession} registered in JNDI with the given name. The
     * {@code PrologSession} will make use of sessions and the session id will be
     * the same as in the {@code HTTPSession}. Use this method in web application
     * servers with support for servlets and {@code HTTPSession} (and when support
     * for sessions is desired). Note: This will cause the {@code PrologSession} to
     * include the session id in its queries.
     *
     * @param name
     *            the name of the prolog session
     * @param httpSession
    ...
    public static PrologSession getPrologSession(String name, HttpSession httpSession) {
        ...
        if (object == null) {
            ...
            httpSession.setAttribute(SESSION_ATTRIBUTE_NAME, new AppSession(session));
            return session;
        } else {
    ...
    /**
     * Sets the port of the Prolog server (default {@code 8066}).
     *
     ...
     * {@link PrologSession#setHost setHost()}).
     *
    ...
    /**
     * Sets the host of the Prolog server (default is {@code localhost}). The host
     * can be specified as either IP-address or host name.
     *
    ...
    /**
     * Set whether to close connections after each send. Default {@code false}.
     *
     * @param close
     *            whether to close after each send
     */
    ...
    /**
     * Sets the connection mode of this {@code PrologSession}. If set to
     * {@code true} it will ensure that it is connected to the Prolog server as soon
     * as a call to {@link PrologSession#executeQuery executeQuery()} or anything
     * else causing a need for communication happens. This is by default set to
     * {@code false}.
     *
     * @param autoConnect
     ...
     * the {@link QueryAnswer QueryAnswer}. Anonymous variables (underscore,
     * {@code _}), will be ignored, and thus not accessible in the
     * {@link QueryAnswer QueryAnswer}. {@code executeQuery} throws
     * {@link java.io.IOException IOException} if communication problems with the
     * server occurs. Please note: {@code executeQuery} will only return one answer.
     *
     * @param query
     ...
     * @throws IllegalCharacterSetException
     *             if the query contains non-ISO-8859-1 characters
    ...
    public QueryAnswer executeQuery(String query) throws IOException, IllegalCharacterSetException {
        return executeQuery(query, new Bindings());
    }

    /**
     * Sends a query to the Prolog server and waits for the answer before returning
     * the {@link QueryAnswer QueryAnswer}. {@code bindings} are variable bindings
     * for the given query and will ensure that the values are stuffed correctly.
     * <p>
     * An example:<br>
     *
     * <pre>
     ...
     *
     * @param query
     *            the query to send to the prolog server The characters in the query
     *            are restricted to ISO-8859-1.
     * @param bindings
     ...
     * @see se.sics.prologbeans.PrologSession#setTimeout
     */
    public QueryAnswer executeQuery(String query, Bindings bindings) throws IOException, IllegalCharacterSetException {
        return executeQuery(query, bindings, prologSessionID);
    }
    ...
    private boolean isValidLatin1(String str) {
     ...
     * @param query
     * @param bindings
     *            {@code null} is allowed, and treated as an empty bindings
     *            instance.
     * @param sessionID
     * @return
     * @throws IOException
     * @throws IllegalCharacterSetException
     */
    private synchronized PBTerm send(String query, Bindings bindings, String sessionID)
            throws IOException, IllegalCharacterSetException {
        if (parentSession != null) {
            return parentSession.send(query, bindings, sessionID);
        }
        if (!isValidLatin1(query)) {
        ...
        try {
            ...
            // Quintus can be confused unless there is whitespace after the full stop.
    ...
    /**
     * Connects to the Prolog server. By default {@link PrologSession#executeQuery
     * executeQuery()} will automatically connect to the server when called.
     *
     * @throws IOException
    ...
    private static class DebugStream extends OutputStream {
    ...
    /**
     * Returns {@code true} if a connection with the Prolog server is open and
    ...
    /**
     * Closes the connection with the Prolog server. The connection can be opened
     * again with {@link PrologSession#connect connect()}.
     */
